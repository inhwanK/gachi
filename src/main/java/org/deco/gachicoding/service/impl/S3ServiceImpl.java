...
public class S3ServiceImpl {
    ...
    public void upload(List<MultipartFile> files, Long boardIdx, String category) throws IOException {
        ...
        for(MultipartFile f : files) {
            ...
            // s3 저장 경로
            String saveFilePath = category + "/" + boardIdx + "/" + origFileName;
    ...
    public List<String> copyTempImage(List<MultipartFile> files) throws IOException {
        ...
        for(MultipartFile f : files) {
            ...
            // 파일 정보 추출 (이걸 dto 생성 시 바로 해버리면?)
            origFileName = f.getOriginalFilename();
            saveFileName = UUID.randomUUID() + "_" + origFileName;

            // 경로에 userId 추가
            
            // s3 저장 경로
            saveFilePath = "temp/" + saveFileName;
    ...
    public void replaceRealPath(List<String> files, Long boardIdx, String category) throws IOException, URISyntaxException {
        String origFileName = null;
        String origFileExtension = null;
        String tamperingFileName = null;
        String saveFileName = null;
        ...
        for(String path : files) {
            ...
            Path p = Paths.get(url.toURI().getPath());
            File file = p.toFile();
            // 파일 정보 추출 (이걸 dto 생성 시 바로 해버리면?)
