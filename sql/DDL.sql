...
CREATE UNIQUE INDEX `UIX_user` ON `gachicoding`.`user` (`user_email` ASC, `user_nick` ASC) VISIBLE;
...
CREATE INDEX `FK_user_TO_gachi_agora` ON `gachicoding`.`gachi_agora` (`user_idx` ASC) VISIBLE;
...
CREATE INDEX `FK_gachi_agora_TO_agora_vote` ON `gachicoding`.`agora_vote` (`agora_idx` ASC) VISIBLE;

CREATE INDEX `FK_user_TO_agora_vote` ON `gachicoding`.`agora_vote` (`user_idx` ASC) VISIBLE;
...
(
    ...
    `board_pin`       TINYINT(1)      NOT NULL DEFAULT '0' COMMENT '고정',
    `board_regdate`   DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '작성일',
    `board_type`      VARCHAR(20)     NOT NULL DEFAULT 'BOARD' COMMENT '유형',
    `board_activated` TINYINT(1)      NOT NULL DEFAULT '1' COMMENT '활성상태',
    `board_thumbnail` VARCHAR(255)    NULL     DEFAULT NULL COMMENT '썸네일',
...
CREATE INDEX `FK_user_TO_board` ON `gachicoding`.`board` (`user_idx` ASC) VISIBLE;
...
CREATE INDEX `FK_user_TO_comment` ON `gachicoding`.`comment` (`user_idx` ASC) VISIBLE;
...
(
    `file_idx`         BIGINT UNSIGNED NOT NULL COMMENT '파일번호',
...
CREATE INDEX `FK_user_TO_gachi_q` ON `gachicoding`.`gachi_q` (`user_idx` ASC) VISIBLE;
...
CREATE INDEX `FK_user_TO_gachi_a` ON `gachicoding`.`gachi_a` (`user_idx` ASC) VISIBLE;

CREATE INDEX `FK_gachi_q_TO_gachi_a` ON `gachicoding`.`gachi_a` (`qs_idx` ASC) VISIBLE;
...
CREATE INDEX `FK_user_TO_social` ON `gachicoding`.`social` (`user_idx` ASC) VISIBLE;
...
(
    ...
    `board_idx`  BIGINT UNSIGNED NOT NULL COMMENT '게시판번호',
    `board_type` VARCHAR(20)     NOT NULL COMMENT '게시판유형',
    PRIMARY KEY (`rel_idx`),
...
CREATE INDEX `FK_tag_TO_tag_rel` ON `gachicoding`.`tag_rel` (`tag_idx` ASC) VISIBLE;
